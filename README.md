...
* Sellers are machine owners that sell their data (i.e. sending data about their vehicle, smart buildings, decentralised car sharing, etc).
* Manufacturers construct a ZKSMP challenge that is answerable each the machine using a mechanism in order to verify the machines authenticity, that the machine was manufactured by them, and that it's identity was not generated by a Random Number Generator (RNG), whilst maintaining privacy of the registered seller data (anonymity of the machine's location, ownership, etc) so that other entities may trust and trade with it.
